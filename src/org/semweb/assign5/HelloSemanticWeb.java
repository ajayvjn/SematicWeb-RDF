...
public class HelloSemanticWeb {
    ...
    private void populateNewFriends() throws IOException {
        ...
        _friends.read(inFoafInstance, null);
        inFoafInstance.close();
    }

    private void mySelf(Model model) {
        // Hello to Me - focused search
        runQuery(" select DISTINCT ?name where { people:me foaf:name ?name}",
    ...
    private void myGmailFriends(Model model) {
        //just get all my gmail friends only - ones with email
